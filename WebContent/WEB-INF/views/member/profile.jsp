					...
					<div class="input-box">
						...
						<div class="profile-image-box">
							<img id="profile-image"
								...
								data-filename="default-profile-image.png"> 
							<input id="profile-image-file-select" type="file" accept=".jpg,.png,.jpeg,.gif,.webp,.bmp">
						</div>
					</div>
					<div class="input-box">
						<label for="">이메일 <label style="font-size: 12px">*수정 가능</label></label>
						<div class="horizontal-input-set">
							<span class="form-control form-input">${empty member.email ? 'NULL' : member.email}</span>
						</div>
					</div>
					<div class="input-box">
						<label for="">비밀번호<label style="font-size: 12px">*수정 가능</label></label> 
						<span class="form-control form-input">${empty member.password ? 'NULL' : member.password}</span>
					</div>
					<div class="input-box">
						<label for="">이름</label> <span class="form-control form-input">${empty member.member_name ? 'NULL' : member.member_name}</span>
					</div>
					<div class="input-box">
						<label for="">전화번호<label style="font-size: 12px">*수정 가능</label></label> 
						<span class="form-control form-input">${empty member.tel ? 'NULL' : member.tel}</span>

					</div>
					<div class="input-box">
						<label for="">주소<label style="font-size: 12px">*수정 가능</label></label> 
						<span class="form-control form-input">${empty member.address ? 'NULL' : member.address}</span>
					</div>
					<div class="input-box">
						<label for="">생년월일<label style="font-size: 12px">*수정 가능</label></label> 
						<span class="form-control form-input">${empty member.birth_date ? 'NULL' : member.birth_date}</span>
					</div>
					<div class="input-box">
						<label>성별<label style="font-size: 12px">* 수정 가능</label></label> 
						<span class="form-control form-input">${empty member.gender ? 'NULL' : member.gender}</span>
					</div>
					<div class="input-box" id="role-input-box">
						<label>역할<label style="font-size: 12px">* 수정 가능</label></label> 
						<span class="form-control form-input">${empty member.role ? 'NULL' : member.role}</span>
					</div>
					<div class="input-box" id="company-input-box">
						<label for="">회사<label style="font-size: 12px">*수정 가능</label></label> 

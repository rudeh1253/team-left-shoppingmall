...
<script>
	...
	window.onload = function() {
		...
		} else if (command === 'sell-chart') {
			classChanger("sell-chart");
		}
	};
</script>
...
<body>
	...
	<div class="d-flex flex-column flex-shrink-0 p-3 bg-light h-100"
		...
		<a href="/" style="padding: 10px 0; justify-content: center;"
			class="d-flex align-items-center fs-5 fw-bold mb-2 mb-lg-0 link-body-emphasis text-decoration-none">Team
			Left</a>
		<hr>
		<ul class="nav nav-pills flex-column mb-auto">
			<li class="nav-item"><a href="#"
				class="nav-link text-dark fw-bold" aria-current="page"> <svg
			...
			<li class="nav-item"><a
				href="/member.do?command=profile&userid=${userid}" id="profile"
				class="nav-link link-dark" aria-current="page"> <svg
						class="bi me-2" width="16" height="16">
						<span id="profile" style="color: #94A5EB"></span></svg> <c:if
						test="${isMyProfile}">내 정보</c:if> <c:if test="${!isMyProfile}">프로필</c:if>
			</a></li>
			<c:if test="${empty role || role == 'buy'}">
				<li class="nav-item"><a
					href="/purchase.do?command=purchase&userid=${ userid }"
					id="buy_list" class="nav-link link-dark" aria-current="page"> <svg
							class="bi me-2" width="16" height="16">
			...
			<c:if test="${not empty role && role == 'sell'}">
				...
				<li class="nav-item"><a href="#" id="seller"
					class="nav-link fw-bold text-dark" aria-current="page"> <svg
							class="bi me-2" width="16" height="16">
							</svg> 판매자
				</a></li>
				<hr>
				<c:if test="${isMyProfile}">
					<li class="nav-item"><a
						href="/purchase.do?command=sell&userid=${ userid }" id="sell_list"
				...
				<li class="nav-item"><a
					...
					id="register_list" class="nav-link link-dark" aria-current="page">
						<svg class="bi me-2" width="16" height="16">
							<span id="show-register-list" style="color: #94A5EB"></span></svg> 상품
						등록 내역
				</a></li>
				<li class="nav-item"><a
					href="/product.do?command=sell-chart&userid=${ userid }"
					id="sell-chart" class="nav-link link-dark" aria-current="page">
						<svg class="bi me-2" width="16" height="16">
							<span id="sell-chart" style="color: #94A5EB"></span></svg> 판매 차트 보기
				</a></li>
			</c:if>
		</ul>
		<hr>
